 ...
 * @inheritDoc
 */
net.XdmRequest.prototype._isRunning = function() {
...
net.XdmRequest.prototype._doSend = function(data, path) {
  ...
  if (this.__request !== null) {
    ...
    this.__request.ontimeout = function() {
      self._handleResult(500);
    };

    this.__request.timeout = 5000;
    ...
    if (fallback.length > 0) {
      url += (url.indexOf('?') === -1 ? '?' : '&') + '__fallback__' + fallback;
    }

    this.__request.open(method, encodeURI(url));
